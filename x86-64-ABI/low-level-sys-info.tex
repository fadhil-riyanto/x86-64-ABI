...
\path{/usr/lib64} or \path{/usr/lib/x86_64-linux-gnu}.  It has to be
...
\begin{itemize}
 ...
 \item For N > 64, they are treated as struct of 64-bit integer
  ...
  the type.  \texttt{_BitInt(N)} types are byte-aligned to 64 bits.
  The size of these types is the
...
(\reg{xmm0} - \reg{xmm31}\footnote{\reg{xmm16} - \reg{xmm31}
...
(\reg{ymm0} - \reg{ymm31}\footnote{\reg{ymm16} - \reg{ymm31}
are only available with Intel AVX-512.}).
...
\item Each field of an object is classified recursively so that always
   two fields \footnote{Zero-width bit-fields are NO_CLASS.} are
   considered.  The resulting class is calculated
...
\reg{xmm16}--\reg{xmm31} & temporary registers & No\\
...
\code{tilecfg} & Tile control register & No \\
    \end{tabular}
...
\subsection{Alternative Function Calling Sequences}

Compilers may support alternative calling sequences with different
performance characteristics.

\begin{enumerate}
\item Compilers that support the \verb|__preserve_most__| function
  attribute use a different convention for preserving registers
  calling or compiling functions annotated with this attribute.

  With this calling convention, registers \RAX to \RDX, \RDI, \RSI,
  \RBP, \RSP, \reg{r8} to \reg{r10}, and \reg{r12} to \reg{r15} are
  not modified by the caller.  Registers used for passing arguments
  are preserved by the called function, but registers used for
  returning results are not.  Functions returning void preserve \RAX.
  The vector and flags registers, and the red zone are not preserved.
  Assumptions regarding CPU states are not affected by this function
  attribute (for example, the direction flag must still be cleared
  when a \verb|__preserve_most__| is called, and the usual stack
  alignment rules apply).

  If a function defined with \verb|__preserve_most__| calls a function
  which lacks the attribute, the first function needs to save the
  registers clobbered by the calling sequence for the second function,
  so that the \verb|__preserve_most__| registers are still preserved
  from the point of view of the first functions' caller.

  Compilers which ignore the \verb|__preserve_most__| attribute for
  function calls produce compatible code, but compiler support is
  required for compiling function definitions.

  If the caller makes use of the \verb|__preserve_most__| attribute,
  this calling convention has limited compatibility with dynamic
  linking.  It is recommended to avoid calling such functions through
  PLT stubs, or to disable lazy binding.  The generic function
  interception interception part of run-time dynamic linker auditing
  may not handle this calling convention correctly.
\end{enumerate}
...
\item[\textindex{Medium code model}]
  ...
  The data sections are split into two parts --- the regular data
  sections still limited in the same way as in the small code model
  and the large data sections having no limits except for available
  addressing space.  The program layout must be set in a way so that
  large data sections (\code{.ldata}, \code{.lrodata}, \code{.lbss})
  are not between regular text and data sections.
...
In the case:
$$\code{l->gp_offset} > 48 - \code{num_gp} * 8$$ or $$\code{l->fp_offset} > 176 - \code{num_fp} * 16$$
